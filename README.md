...
1. [Прошивка (Flashing)](#прошивка-flashing)
2. [Слои и их назначение](#слои-и-их-назначение)
3. [Схематическое отображение слоев](#схематическое-отображение-слоев)
4. [Home Row Mods](#home-row-mods)
5. [Tap Dance](#tap-dance)
6. [Комбинации (Combos)](#комбинации-combos)
7. [Макросы](#макросы)
8. [Советы по использованию](#советы-по-использованию)

## Прошивка (Flashing)

### Шаг 1 — Получить прошивку

Прошивка собирается автоматически в GitHub Actions при каждом push в репозиторий.

1. Открыть вкладку **Actions** в репозитории на GitHub
2. Выбрать последний успешный запуск **ZMK Firmware**
3. Скачать артефакт **`firmware-charybdis-qwerty`** (zip-архив с обоими файлами)

Внутри архива:
| Файл | Половина |
|------|----------|
| `charybdis_qwerty_right.uf2` | Правая (central, с трекболом) |
| `charybdis_qwerty_left.uf2` | Левая (peripheral) |

> Также есть `firmware-reset-nanov2` → `firmware_reset_nano_v2.uf2` — для сброса паринга BT (используется редко).

---

### Шаг 2 — Войти в режим загрузчика (без физической кнопки)

Клавиши `&bootloader` доступны через раскладку — физически нажимать кнопку Reset не нужно.

**Правая половина** (прошивать первой — она подключается по USB):

```
Удержать  Mo NAV  (нижняя правая клавиша большого пальца)
Нажать    7       (верхний ряд, цифра 7)
```

**Левая половина:**

```
Удержать  Space   (средняя клавиша левого большого пальца)
Нажать    Shift   (левый нижний угол, клавиша Shift/Caps)
```

> Альтернатива для обеих половин — ADJUST слой (LOWER + SYMB одновременно):  
> вторая клавиша нижнего ряда левой половины = `&bootloader`.

После входа в загрузчик половина появляется как USB-диск (`NICENANO`).

---

### Шаг 3 — Залить прошивку

1. **Правая половина** — подключить по USB к компьютеру → войти в загрузчик → скопировать `charybdis_qwerty_right.uf2` на диск `NICENANO` → диск отключится автоматически
2. **Левая половина** — подключить по USB → войти в загрузчик → скопировать `charybdis_qwerty_left.uf2` → диск отключится

После прошивки обе половины перезагрузятся и соединятся между собой по BLE автоматически.

---

### Сброс BT паринга (если половины не соединяются)

Прошить обе половины файлом `firmware_reset_nano_v2.uf2`, затем перепрошить штатной прошивкой.
...
- **Доступ**: **Автоматически** при движении трекбола (деактивируется через 500 мс после остановки); `&tog MOUSE` на больших пальцах для ручной фиксации
...
- `←/↓/↑/→` - Движение курсора (через кнопки, трекбол работает всегда)
- `LClk/MClk/RClk` - Левая/средняя/правая кнопки мыши
- `TogM` - Ручная фиксация мышиного слоя (для перетаскивания и длительной работы с мышью)

> **Автоактивация**: слой включается автоматически при движении трекбола и выключается через 500 мс после остановки. Кнопки LClk/RClk доступны напрямую на основном слое (большие пальцы правой руки), поэтому клики работают без ручного переключения.
...
### dt_grave (левая половина, клавиша `` ` ``)
- **1 нажатие**: Backtick/Tilde (`` ` ``/`~`)
- **5 нажатий**: Очистка Bluetooth (BT_CLR) — сброс сопряжения текущего устройства

### dt_lock (правая половина, клавиша блокировки экрана)
- **1 нажатие**: Блокировка экрана (Ctrl+Cmd+Q)
- **5 нажатий**: Очистка Bluetooth (BT_CLR) — сброс сопряжения текущего устройства
...
Трекбол расположен на правой половине клавиатуры и управляется через драйвер `pixart,pmw3610-alt` (badjeff zmk-pmw3610-driver).

- **Основное движение**: управление курсором мыши
- **Прокрутка**: активируется на слое MOUSE (удержание кнопки прокрутки или переключение слоя)
- Конфигурация: `CONFIG_PMW3610_ALT=y` в `charybdis_right.conf`

> **Примечание**: `input_listener` (trackball_listener) должен быть определён только в правом оверлее (`charybdis_right.overlay`), так как трекбол физически присутствует только на правой половине.
